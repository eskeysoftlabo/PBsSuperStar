...
eq(C[18].name.text, "クラスマスタリー", "Class Mastery starts the next column")
eq(C[19].name.text, "Skill 1")
eq(C[21].name.text, "", "a blank row separates Class Mastery from Mundus")
eq(C[22].name.text, "ムンダス")
eq(C[23].name.text, "Boon"); eq(C[23].icon.hidden, false, "mundus keeps its icon")
eq(C[25].name.text, "呪い")
U:MoveColumn(1)
eq(U.column, 2)
