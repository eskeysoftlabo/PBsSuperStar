...
eq(find(gear, "HEAD").subline, "1　1　enchant", "armour weight, then trait, then enchantment")
...
-- L1/R1 on the first page walks the columns on screen, keeping the row.
U.column = 1; U.selected[1] = 3; U:Render()
U:MoveGridColumn(1); eq(U.column, 2, "R1 from equipment goes to the build")
eq(U.data[2][U.selected[2]].key, "cpslot2", "same row: third row of the first build column")
U:MoveGridColumn(1); eq(U.data[2][U.selected[2]].key, "classLine1", "a blank row takes the entry above it")
U:MoveGridColumn(1); eq(U.data[2][U.selected[2]].key, "classLine1", "nothing right of the build's second column")
U:MoveGridColumn(-1); eq(U.data[2][U.selected[2]].key, "cpslot1")
U:MoveGridColumn(-1); eq(U.column, 1, "L1 from the build's first column goes back to equipment")
eq(U.selected[1], 2)
U:MoveGridColumn(-1); eq(U.column, 1, "nothing left of equipment")
U.column = 2; U.selected[2] = 1; U:Render()
